...
namespace SME.Sondagem.Aplicacao.UseCases.Questionario.Relatorio
{
    public class ObterSondagemRelatorioConsolidadoAnoUseCase : ObterSondagemRelatorioConsolidadoBase, IObterSondagemRelatorioConsolidadoAnoUseCase
    {
        ...
        {
            var respostas = await RepositorioSondagem.RepositorioRespostaAluno.ObterRespostasParaRelatorioConsolidadoAsync(filtro, cancellationToken);

            if (respostas == null || !respostas.Any())
                return new RelatorioConsolidadoSondagemDto { Titulo = "Relatório Consolidado por Ano - Sem Dados" };

            RelatorioConsolidadoSondagemDto relatorio = new()
            {
                Titulo = $"Relatório Consolidado de Sondagem por Ano - {filtro.AnoLetivo}"
            };

            var agrupamentoPorAnoTurma = respostas.GroupBy(r => new {r.AnoTurma,r.QuestaoId, r.QuestaoNome })
                                        .OrderBy(g => g.Key.QuestaoNome);

            relatorio.Questoes = [.. agrupamentoPorAnoTurma.Select(g => ProcessarQuestao(g.Key.QuestaoId, g.Key.QuestaoNome, [.. g]))];
