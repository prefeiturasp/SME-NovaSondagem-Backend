...
public class AtualizarContextoRespostasLegadoUseCaseTeste
{
    ...
    [Fact]
    public async Task ExecutarAsync_Quando_Estudante_Foi_Remanejado_Apos_Inicio_Da_Sondagem_Deve_Ignorar_E_Notificar_Zero_Atualizacoes()
    {
        var repositorioResposta = new Mock<IRepositorioRespostaAluno>();
        var repositorioSondagem = new Mock<IRepositorioSondagem>();
        var dadosAlunosService = new Mock<IDadosAlunosService>();
        ...
        var dataInicioSondagem = new DateTime(2026, 3, 1, 0, 0, 0, DateTimeKind.Utc);
        repositorioResposta
            .Setup(r => r.ObterRespostasSemContextoPaginadoAsync(
                It.IsAny<int>(), It.IsAny<int>(), It.IsAny<int>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<RespostaAlunoLegadoDto>
        ...
        repositorioSondagem
            .Setup(s => s.ObterPorIdAsync(1, It.IsAny<CancellationToken>()))
            .ReturnsAsync(CriarSondagem(1, dataInicioSondagem));
        ...
        dadosAlunosService
            ...
            .ReturnsAsync(new List<AlunoEolDto>
            {
                new()
                {
                    ...
                    CodigoTurma = 50,
                    CodigoEscola = "UE1",
                    DataMatricula = new DateTime(2026, 5, 1, 0, 0, 0, DateTimeKind.Utc),
                },
            });

        dadosAlunosService
            .Setup(s => s.ObterDadosRacaGeneroAlunos(It.IsAny<int>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Array.Empty<AlunoRacaGeneroDto>());
        ...
        elasticAluno
            .Setup(a => a.ObterAlunosPorIdTurma(It.IsAny<int>(), It.IsAny<int>(), It.IsAny<CancellationToken>()))
